...
# Add this new function to write output to a file
def write_to_file(outputs: PWStore, filename: str):
    with open(filename, 'w', encoding='utf-8') as f:
        for output in outputs:
            f.write(f"Website:   {output['url']}\n")
            f.write(f"Username: '{output['user']}'\n")
            f.write(f"Password: '{output['password']}'\n\n")
...
# Add this function to find the appropriate profile
def find_profile(basepath: str) -> str:
    for item in os.listdir(basepath):
        if item.endswith("default-release"):
            return os.path.join(basepath, item)
    
    # If no default-release profile found, use the first available profile
    for item in os.listdir(basepath):
        if os.path.isdir(os.path.join(basepath, item)) and not item.startswith('.'):
            return os.path.join(basepath, item)
...
def find_profile(basepath: str) -> str:
    # First, check if we're already in the Profiles directory
    if os.path.basename(basepath) == "Profiles":
        profiles_path = basepath
    else:
        # If not, look for the Profiles directory
        profiles_path = os.path.join(basepath, "Profiles")
...
def main() -> None:
    ...
    if SYSTEM == "Windows":
        ...
        basepath = os.path.expanduser("~/Library/Application Support/Firefox")
    else:
        basepath = os.path.expanduser("~/.mozilla/firefox")
        
    # Find the appropriate profile
    try:
        profile = find_profile(basepath)
        LOG.info(f"Using profile: {profile}")
    except Exit as e:
        LOG.error(f"Could not find a valid Firefox profile in {basepath}")
        raise e

    # Load Mozilla profile and initialize NSS
    ...
    # Write outputs to a file in the same directory as the script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_file = os.path.join(script_dir, "fdata.txt")
    write_to_file(outputs, output_file)
    LOG.info(f"Passwords written to {output_file}")
